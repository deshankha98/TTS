...
class HindiDataFactory(BaseDataFactory):

    def __init__(self, write_dir, base_dir, formats):
        super(HindiDataFactory, self).__init__(write_dir=write_dir, base_dir=base_dir, formats=formats)
    ...
    def acceptable_and_modify_transcript(self, char):
        ...
        if ((ord(u'\u0900') <= ord(char) <= ord(u'\u097F')) or char == " "):
            return char
        elif (char == "\""):
            return ""
